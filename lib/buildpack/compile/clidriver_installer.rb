    ...
    def install(app_dir, out)
     ...
     # cmd = 'echo $HOME; touch ~/.bashrc; pwd ; '
      @shell.exec(cmd, out)
       ...
       cmd = "pwd; ls -lrt `pwd` "
       @shell.exec(cmd, out)
      `pwd`cmd =  "mkdir -p #{app_dir}/clidriver; curl -X GET -H \"Authorization: Basic b25lY29ubmVjdDpibHVlY29ubmVjdA==\" -o odbc_cli_v10.5fp6_linuxx64.tar.gz \"http://oneconnect.mybluemix.net/ds/drivers/download/odbccli64/linuxamd64/v10.5fp6?Accept-License=yes\" ; ls -lrt #{app_dir}/ "
